...
export class ClassIntroduction extends Component<ClassIntroductionProps, ClassIntroductionState> {
    ...
    render() {
        return (
            <div>
                <img className={styles.ClassPictureMobile + " " + pictureAnimation.PictureMobileOnly} alt="Class" src={`${process.env.PUBLIC_URL}/assets/Classes/${this.props.characterClass.name}/Picture.webp`}/>
                {this.getInformation()}
            </div>
        )
    }
